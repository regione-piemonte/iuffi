package it.csi.iuffi.iuffiweb.presentation.quadro.pianoselettivocapriolo;
...
@Controller
@IuffiSecurity(value = "CU-IUFFI-2006-M", controllo = IuffiSecurity.Controllo.NESSUNO)
@RequestMapping(value = "cuiuffi2006m")
public class CUIUFFI2006MModificaDistretto extends BaseController {

  public static final String CU_NAME = "CU-IUFFI-2006-M";
  ...
  public String index(@PathVariable("tipo") String tipo, @PathVariable("idDistretto") long idDistretto, @PathVariable("idPianoDistrettoOgur") long idPianoDistrettoOgur, Model model, HttpServletRequest request, HttpSession session) throws InternalUnexpectedException {
    ...
    return "pianoselettivocaprioli/modificadistretto";
  }
  ...
  public String confermaModifica(Model model, HttpSession session, HttpServletRequest request) throws InternalUnexpectedException { 
    ...
    if(censito1 != 0) {
      percentuale1 = (new BigDecimal(prelievo1).divide(new BigDecimal(prelievo1+prelievo2+prelievo3+indeterminatiP), 2, RoundingMode.HALF_UP));
    }
    else {
      percentuale1 = new BigDecimal("0");
    }
    if(censito2 != 0) {
      percentuale2 = (new BigDecimal(prelievo2).divide(new BigDecimal(prelievo1+prelievo2+prelievo3+indeterminatiP), 2, RoundingMode.HALF_UP));
    }
    else {
      percentuale2 = new BigDecimal("0");
    }
    if(censito3 != 0) {
      percentuale3 = (new BigDecimal(prelievo3).divide(new BigDecimal(prelievo1+prelievo2+prelievo3+indeterminatiP), 2, RoundingMode.HALF_UP));
    }
    else {
    ...
    if(new BigDecimal(prelievo1).compareTo(a1.multiply(new BigDecimal(prelievo1+prelievo2+prelievo3+indeterminatiP))) > 0 || new BigDecimal(prelievo1).compareTo(da1.multiply(new BigDecimal(prelievo1+prelievo2+prelievo3+indeterminatiP))) < 0) {
      esito1 = "N";
    }
    else {
      esito1 = "S";
    }
    if(new BigDecimal(prelievo2).compareTo(a2.multiply(new BigDecimal(prelievo1+prelievo2+prelievo3+indeterminatiP))) > 0 || new BigDecimal(prelievo2).compareTo(da2.multiply(new BigDecimal(prelievo1+prelievo2+prelievo3+indeterminatiP))) < 0) {
      esito2 = "N";
    }
    else {
      esito2 = "S";
    }
    if(new BigDecimal(prelievo3).compareTo(a3.multiply(new BigDecimal(prelievo1+prelievo2+prelievo3+indeterminatiP))) > 0 || new BigDecimal(prelievo3).compareTo(da3.multiply(new BigDecimal(prelievo1+prelievo2+prelievo3+indeterminatiP))) < 0) {
      esito3 = "N";
    }
    else {
      esito3 = "S";
    }
    if(prelievo1+prelievo2+prelievo3+indeterminatiP <= max) {
      esitoTot = "S";
    }
    else {
      esitoTot = "N";
    ...
    for (Object o : Collections.list(iere)) {
      ...
      if (!errors.isEmpty()) {
        ...
        quadroIuffiEJB.updatePianoDistrettoOgur(new Long(idPiano), censito1+censito2+censito3+indetC, indetC, 
            prelievo1+prelievo2+prelievo3+indeterminatiP, indeterminatiP, 
            new BigDecimal(censito2+censito3+indetC).multiply(new BigDecimal(percMax).divide(new BigDecimal(100))), esitoTot, indeterminatiP);
        ...
        return "redirect:../cuiuffi2006v/index_"+idDistretto+"_"+tipo+"_"+idPiano+".do";
      }
    }
    ...
    @RequestMapping(value = "/esito_{prelievo}_{c1}_{c2}_{c3}_{indp}_{arg1}", method = RequestMethod.GET, produces = "application/text")
    @ResponseBody
    public String esito(Model model,
        ...
        @PathVariable("indp") String indp, @PathVariable("arg1") String arg1,
      ...
      if(da != null && a != null ) {
        ...
        if(prel.compareTo(a.multiply(prelievo1.add(prelievo2.add(prelievo3.add(indet))))) > 0 || prel.compareTo(da.multiply(prelievo1.add(prelievo2.add(prelievo3.add(indet))))) < 0) {
          esito = "ko";
        }
        else {
          esito = "ok";
