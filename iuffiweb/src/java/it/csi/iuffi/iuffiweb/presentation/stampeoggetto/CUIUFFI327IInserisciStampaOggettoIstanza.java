...
@Controller
@IuffiSecurity(value = "CU-IUFFI-327-I", controllo = IuffiSecurity.Controllo.PROCEDIMENTO)
@RequestMapping("/cuiuffi327i")
@IsPopup
public class CUIUFFI327IInserisciStampaOggettoIstanza extends BaseController
{
  @Autowired
      ...
      throws InternalUnexpectedException
  {
    ...
    return "stampeoggetto/allegaDocumentoIstanza";
  }

  public void loadTipologie(Model model, HttpSession session) throws InternalUnexpectedException
  { 
    List<StampaOggettoIconaDTO> tipologie = quadroEJB.getElencoDocumentiStampeDaAllegare(getIdProcedimentoOggetto(session), null,IuffiConstants.USECASE.STAMPE_OGGETTO.INSERISCI_STAMPA_OGGETTO_ISTANZA);
      ...
      @RequestParam(value = "fileDaAllegare", required = false) MultipartFile fileAllegato) throws InternalUnexpectedException, IOException
  {
    ...
    {
      List<StampaOggettoIconaDTO> tipoSelezionato = quadroEJB.getElencoDocumentiStampeDaAllegare(idProcedimentoOggetto, lIdTipoDocumento,IuffiConstants.USECASE.STAMPE_OGGETTO.INSERISCI_STAMPA_OGGETTO_ISTANZA);
      if (tipoSelezionato != null && !tipoSelezionato.isEmpty())
      {
        oggettoIconaDTO = tipoSelezionato.get(0);
        
        if (utenteAbilitazioni.getRuolo().isUtenteIntermediario() && IuffiConstants.FLAGS.SI.equals(oggettoIconaDTO.getFlagFirmaGrafometrica()))
        {
    ...
    if (!hasErrors)
    {
      AgriWellDocumentoVO agriWellDocumentoVO = getAgriWellDocumentoVO(quadroEJB, session, oggettoIconaDTO, fileAllegato.getBytes(), fileAllegato.getOriginalFilename(), utenteAbilitazioni, idProcedimentoOggetto);
     // AgriWellDocumentoVO agriWellDocumentoVO = getAgriWellDocumentoVO(session, lIdTipoDocumento, fileAllegato, oggettoIconaDTO, utenteAbilitazioni);
      ...
      stampa.setIdStatoStampa((IuffiConstants.FLAGS.SI.equals(oggettoIconaDTO.getFlagDaFirmare()))? IuffiConstants.STATO.STAMPA.ID.FIRMATO_DIGITALMENTE: IuffiConstants.STATO.STAMPA.ID.STAMPA_ALLEGATA);

      // recupero iddocindex del docuemnto adre che è stato sostituito da questo qua allegato
      List<StampaOggettoDTO> elenco = quadroEJB.getElencoStampeOggetto(idProcedimentoOggetto, null);
      Long idDocIndexPadre = null;
      for(StampaOggettoDTO item:elenco)
      {
        if(item.getExtIdTipoDocumento() == oggettoIconaDTO.getExtIdTipoDocumento())
        {
          idDocIndexPadre = item.getExtIdDocumentoIndex();
        }
      }
      
      String error = quadroEJB.aggiungiOggettoStampa(stampa, agriWellDocumentoVO, getUtenteAbilitazioni(session),idDocIndexPadre);
      if (error!=null)
      {
  ...
  private AgriWellDocumentoVO getAgriWellDocumentoVO(IQuadroEJB quadroEJB, HttpSession session, StampaOggettoIconaDTO oggettoIconaDTO, byte[] pdf, String fileName,
      ...
      throws InternalUnexpectedException, IOException
  {
    ...
    { IuffiConstants.PARAMETRO.DOQUIAGRI_CARTELLA, IuffiConstants.PARAMETRO.DOQUIAGRI_CLASS_REG, IuffiConstants.PARAMETRO.DOQUIAGRI_FASCICOLA });
