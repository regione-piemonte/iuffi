...
public class TrappolaOnExcelBuilder extends AbstractXlsView {
  ...
  public void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    String filename = "trappoleON.xls";
    // change the file name
    response.setHeader("Content-Disposition", "attachment; filename=\""+filename+"\"");

    List<TrappolaOnDTO> trappoleOn = (List<TrappolaOnDTO>) model.get("elenco");
    //MissioneRequest mr = (MissioneRequest) model.get("missioneRequest");
      
    // create excel xls sheet
    Sheet sheet = workbook.createSheet("Lista Trappole Organismo Nocivo");
    ...
    titleRow.createCell(0).setCellValue("Lista Trappole Organismo Nocivo");
    ...
    header.createCell(1).setCellValue("Organismo Nocivo");
    ...
    for(TrappolaOnDTO trappolaOn : trappoleOn) {
      Row row = sheet.createRow(rowCount++);
      row.createCell(0).setCellValue(trappolaOn.getDescrTipoTrappola());
      row.createCell(1).setCellValue(trappolaOn.getDescrOn());
      if(trappolaOn.getDataInizioValidita() != null) {
        row.createCell(2).setCellValue(sdfData.format(trappolaOn.getDataInizioValidita()) + " " + sdfOra.format(trappolaOn.getDataInizioValidita()));
      }
      if(trappolaOn.getDataFineValidita() != null) {
        row.createCell(3).setCellValue(sdfData.format(trappolaOn.getDataFineValidita()) + " " + sdfOra.format(trappolaOn.getDataFineValidita()));
      }
    }
