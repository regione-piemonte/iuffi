...
public class TrappolaEnteExcelBuilder extends AbstractXlsView {
  ...
  public void buildExcelDocument(Map<String, Object> model, Workbook workbook, HttpServletRequest request, HttpServletResponse response) throws Exception {
    ...
    String filename = "trappoleEnte.xls";
    // change the file name
    response.setHeader("Content-Disposition", "attachment; filename=\""+filename+"\"");

    List<TrappolaEnteDTO> trappoleEnte = (List<TrappolaEnteDTO>) model.get("elenco");
    ...
    for(TrappolaEnteDTO trappolaEnte : trappoleEnte) {
      Row row = sheet.createRow(rowCount++);
      row.createCell(0).setCellValue(trappolaEnte.getDescrTipoTrappola());
      row.createCell(1).setCellValue(trappolaEnte.getDescrEnte());
      BigDecimal bd = trappolaEnte.getCosto();
      double d = bd.doubleValue();
      row.createCell(2).setCellValue(d);
      if(trappolaEnte.getDataInizioValidita() != null) {
        row.createCell(3).setCellValue(sdfData.format(trappolaEnte.getDataInizioValidita()) + " " + sdfOra.format(trappolaEnte.getDataInizioValidita()));
      }
      if(trappolaEnte.getDataFineValidita() != null) {
        row.createCell(4).setCellValue(sdfData.format(trappolaEnte.getDataFineValidita()) + " " + sdfOra.format(trappolaEnte.getDataFineValidita()));
      }
    }
